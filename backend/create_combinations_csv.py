...
# Creates a csv with all 495 combinations of who the top 8 third places will face in the knockout stage
def create_csv_from_string(text_data, output_filename="output.csv", headers=None):
    # Split the string into a list of words using spaces
    words = text_data.split()
    
    # Define the required number of columns
    num_cols = 9
    ...
    if headers is None:
        ...
        raise ValueError(f"Please provide exactly {num_cols} headers.")
        
    # Chunk the list of words into rows of 9
    rows = [words[i:i + num_cols] for i in range(0, len(words), num_cols)]
    
    # Optional check: Let you know if the word count doesn't perfectly match 495 rows
    if len(rows) != 495:
        print(f"Note: Generated {len(rows)} data rows instead of the expected 495. "
              f"Check your string length (currently {len(words)} words).")

    # Write the data to a CSV file
    with open(output_filename, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        
        # Write the 1 header row
        writer.writerow(headers)
        
        # Write the 495 data rows
...
# 1. This is where your large string goes
my_large_string = """
...
# 2. (Optional) Define custom headers for your 9 columns
my_headers = ["Option", "1A", "1B", "1D", "1E", "1G", "1I", "1K", "1L"]

# 3. Run the function
create_csv_from_string(
    text_data=my_large_string, 
    output_filename="my_parsed_data.csv", 
    headers=my_headers
)